...
**Recording.** Token-2022 calls the hook on every $ROCK transfer. The hook classifies it from the curve's base vault: tokens leaving the vault are a buy, tokens entering it are a sell, anything else is a send. Buys worth less than 0.1 SOL are not recorded. The ledger has two rings that share one sequence number. Buys (buyer, amount, value in SOL at the curve's current price, slot) go to a 2,048-entry buy ring. Sells and sends (sender only) go to a 4,096-entry out ring. Sends are nearly free, so anyone can flood the out ring, but the flood can never push a buy out of its ring. At worst some sells are never read, and those sellers keep their Rockies lit. The hook never mints or calls another program and only uses fixed extra accounts, so it stays cheap and fits inside aggregator routes.

**Crank.** `process_buy`, `process_outs` and `process_skip` read both rings in sequence order. Anyone can call them. A buy becomes a `Ticket` with a number and a tier set by the $ROCK it received. `process_outs` handles up to a few hundred sells and sends in one instruction; each flags every earlier ticket of its wallet as burnt out. A buy that lands on a listed router account and is passed on to a user in the same slot is credited to that user. If a ring wraps past unread entries, `recover_overflow` counts them as lost and moves on.
...
1. `finalize` checks the hook is gone and every ledger entry is processed, then fixes the two Supernovas that are facts on the chain: the buy that filled the curve (the last ticketed buy, if the final one was under 0.1 SOL), and the biggest buy of the wallet with the largest total bought on the curve. If that biggest buy is also the one that filled the curve, the second Supernova goes to the runner-up's biggest buy instead. It also picks a draw slot 8 slots ahead.
...
5. `thaw` removes the freeze, and Rockies trade like any other Core NFT. It only works once every Supernova is crowned, so nobody can buy a winner from an owner who doesn't know yet.
6. `close_ledger` lets the admin reclaim the ledger's rent (about 2.85 SOL).
...
npm run test:spam        # a flood of sends overruns the out ring; every buy still gets its Rocky
...
Every instruction the bot sends is permissionless, so anyone can run it. The bot only pays fees and rent: about 0.006 SOL per Rocky. It can be stopped and restarted at any time, because all progress is stored on-chain. The limit is the ledger's rings. If more than 2,048 buys, or 4,096 sells and sends, pile up while nobody runs the crank, the oldest unread ones are overwritten; the bot then calls `recover_overflow` and moves on. Optional settings are `PRIORITY_MICROLAMPORTS` (default 50000) and `TICK_MS` (default 2000).
